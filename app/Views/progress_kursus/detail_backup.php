                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="py-4 px-4">
                            <div class="flex justify-center gap-2">
                                ...
                                <button class="btnHapusPertemuan btn-hover px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-xs font-semibold hover:bg-red-200 transition-all"
                                        data-id="<?= $d['id'] ?>"
                                        data-pertemuan="<?= $d['pertemuan_ke'] ?>">
    ...
    <div class="md:hidden space-y-4">
        ...
        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <div class="grid grid-cols-2 gap-2 mt-3">
                <button class="btnEditPertemuan px-3 py-2 bg-yellow-100 text-yellow-700 rounded-lg text-sm font-semibold hover:bg-yellow-200 transition-all"
                        data-id="<?= $d['id'] ?>"
                        data-pertemuan="<?= $d['pertemuan_ke'] ?>"
                        data-tanggal="<?= $d['tanggal'] ?>"
                        data-materi="<?= esc($d['materi'], 'attr') ?>"
                ...
                <button class="btnHapusPertemuan px-3 py-2 bg-red-100 text-red-700 rounded-lg text-sm font-semibold hover:bg-red-200 transition-all"
                        data-id="<?= $d['id'] ?>"
                        data-pertemuan="<?= $d['pertemuan_ke'] ?>">
...
<script>
    ...
    // ==================== HAPUS PERTEMUAN ====================
    document.querySelectorAll(".btnHapusPertemuan").forEach(btn => {
        btn.addEventListener("click", function() {
            const detailIdValue = this.dataset.id;
            const pertemuanKe = this.dataset.pertemuan;
            const progressId = <?= $progress['id'] ?>;

            Swal.fire({
                title: 'Hapus Pertemuan?',
                html: `Apakah Anda yakin ingin menghapus data <strong>Pertemuan Ke-${pertemuanKe}</strong>?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`<?= base_url('progress-kursus/detail/') ?>${progressId}/delete/${detailIdValue}`, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil!',
                                text: 'Pertemuan berhasil dihapus.',
                                showConfirmButton: false,
                                timer: 1500,
                                timerProgressBar: true
                            }).then(() => {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Gagal!',
                                text: data.message || 'Gagal menghapus pertemuan.',
                                confirmButtonColor: '#667eea'
                            });
                        }
                    })
                    .catch(error => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: 'Terjadi kesalahan saat menghapus pertemuan.',
                            confirmButtonColor: '#667eea'
                        });
                    });
                }
